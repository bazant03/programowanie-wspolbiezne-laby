...
class calkowanie_m_prostokatow {
    private:
        ...
        double *wartosc; //wskaźnik do zmiennej, w której należy zapisać wynik
        int numer_watku; //!!!!!!!!!!!

    public:
        //kostruktor ustawia wartości pól
        calkowanie_m_prostokatow(double _a, double _b, double *_wartosc, int _numer_watku) :
            a(_a), b(_b), wartosc(_wartosc), numer_watku(_numer_watku) {}
...
int main(int argc, char *argv[]) {
    ...
    calkowanie_m_prostokatow calka(a, b, &wynik, 100); //!!!!!!!!!!!!!!!!!!!!!!!!
    //tworzenie wątku
    thread watek(calka);
